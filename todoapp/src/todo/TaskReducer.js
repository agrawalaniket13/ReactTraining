...
const TaskReducer = () => {
    ...
    function reducer(todos, act) 
    {
        switch (act.type) 
        {
            case ACTIONS.ADD_TODO:
                return [...todos, { id: Date.now(), name: act.payload.todo, complete: false }]
            case ACTIONS.DELETE_TODO:
                return todos.filter(todo => todo.id != act.payload.id)
            case ACTIONS.TOGGLE_TODO:
                return todos.map(todo => {
                    if (todo.id === act.payload.id)
